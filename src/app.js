...
const App = () => {
  const [applianceStatus, setApplianceStatus] = useState({ light: false, fan: false });

  useEffect(() => {
    axios.get("http://localhost:5000/status").then((res) => setApplianceStatus(res.data));
  }, []);

  const toggleAppliance = (name) => {
    axios.post("http://localhost:5000/toggle", { appliance: name }).then((res) => {
      setApplianceStatus(res.data.status);
    });
  };
